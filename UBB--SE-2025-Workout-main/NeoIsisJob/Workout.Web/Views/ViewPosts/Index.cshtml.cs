...
public class IndexModel : PageModel
{
    ...
    public async Task<JsonResult> OnPostReactAjax(long postId, string type)
    {
        try
        {
            ...
            var existing = await _reactionRepository.GetReaction(userId, postId);

            if (existing == null)
            {
                await _reactionRepository.Add(new Reaction { UserId = userId, PostId = postId, Type = reactionType });
            }
            else if (existing.Type == reactionType)
            {
                await _reactionRepository.Delete(userId, postId);
            }
            else
            {
                await _reactionRepository.Update(userId, postId, reactionType);
            }

            var reactions = await _reactionRepository.GetReactionsByPostId(postId);
    ...
    public async Task<IActionResult> OnPostToggleReaction(long postId, string type)
    {
